...
void stu_bitwise(std::span<std::int8_t> result, std::span<const std::int8_t> a,
    ...
    constexpr std::uint64_t C1 = 0xA5A5A5A5A5A5A5A5ULL; 
    constexpr std::uint64_t C2 = 0x9999999999999999ULL; 
    std::size_t i = 0;
    const std::size_t n64 = n / 8;
    for (; i + 3 < n64; i += 4)
    {
        res64[i] = C1 ^ ((a64[i] | b64[i]) & C2);
        res64[i + 1] = C1 ^ ((a64[i + 1] | b64[i + 1]) & C2);
        res64[i + 2] = C1 ^ ((a64[i + 2] | b64[i + 2]) & C2);
        res64[i + 3] = C1 ^ ((a64[i + 3] | b64[i + 3]) & C2);
    }
    for (; i < n64; ++i) res64[i] = C1 ^ ((a64[i] | b64[i]) & C2);
    constexpr std::uint8_t c1_8 = 0xA5u;
    constexpr std::uint8_t c2_8 = 0x99u;
    for (std::size_t pos = n64 * 8; pos < n; ++pos)
    {
        const auto ua = static_cast<std::uint8_t>(a[pos]);
        const auto ub = static_cast<std::uint8_t>(b[pos]);
        result[pos] = static_cast<std::int8_t>(c1_8 ^ ((ua | ub) & c2_8));
    }
}
