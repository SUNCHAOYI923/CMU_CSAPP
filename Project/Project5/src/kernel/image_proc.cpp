...
void stu_image_proc(image_proc_args& args) {
    const size_t w = args.width;
    const size_t h = args.height;
    float* out = args.output.data();
    const float* r = args.r_channel.data();
    const float* g = args.g_channel.data();
    const float* b = args.b_channel.data();
    const float threshold = args.threshold;
    ...
    for (size_t y = 0; y < h; ++y)
    {
        for (size_t x = 0; x < w; ++x)
        {
            ...
            adjusted = (adjusted < 0.0f) ? 0.0f : (adjusted > 1.0f) ? 1.0f : adjusted;
            ...
            float hdr_result = grayEnhance * gain;
