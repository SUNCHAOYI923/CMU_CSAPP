...
void stu_trace_replay(uint64_t& out,
    ...
    const size_t PREFETCH_DIS = 64;
    size_t pos = 0;
    while (pos + PREFETCH_DIS * 2 < n)
    {
        uint32_t idx1 = trace[pos + PREFETCH_DIS];
        uint32_t idx2 = trace[pos + PREFETCH_DIS * 2];
        __builtin_prefetch(&records[idx1], 0, 3);
        __builtin_prefetch(&records[idx2], 0, 3);
        uint64_t cost = trace_replay_cost(records[trace[pos]]);
