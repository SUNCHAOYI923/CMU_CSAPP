...
void stu_csr_spmm(const CSRMatrix &csr, const std::vector<float> &dense_t,
    ...
    {
        const float* src_row = &dense_t[i * cols];
        for (size_t j = 0; j < cols; ++j) D[j * dense_cols + i] = src_row[j];
    }
    ...
    for (size_t r = 0; r < rows; ++r)
    {
        ...
        for (int p = p_start; p < p_end; ++p)
        {
            ...
            const float* D_row = &D[c * dense_cols];
            for (size_t n = 0; n < dense_cols; ++n) out_row[n] += val * D_row[n];
        }
    }
}
