...
static inline float fast_log(float x)
{
    ...
    float y2 = y * y;
    return 2.0f * y * (1.0f + y2 * (0.3333333333f + y2 * 0.2000000000f));
}
...
static inline float fast_norm_cdf(float x)
{
    bool is_positive = (x >= 0.0f);
    x = is_positive ? x : -x;
    float t = 1.0f / (1.0f + 0.2316419f * x);
    float poly = t * (0.319381530f + t * (-0.356563782f + t * (1.781477937f + t * (-1.821255978f + t * 1.330274429f))));
    float cdf = 1.0f - poly * (0.3989422804f * expf(-0.5f * x * x));
    return is_positive ? cdf : (1.0f - cdf);
}
...
void stu_BlkSchls(std::vector<float> &CallOptionPrice,
    ...
    const float* p_spot = spotPrice.data();
    const float* p_strike = strike.data();
    const float* p_rate = rate.data();
    const float* p_vol = volatility.data();
    const float* p_time = time.data();
    float* p_call = CallOptionPrice.data();
    float* p_put = PutOptionPrice.data();
    for (size_t i = 0; i < n; ++i)
    {
        float spot = p_spot[i];
        float strike_val = p_strike[i];
        float rt = p_rate[i];
        float vol = p_vol[i];
        float t = p_time[i];
        float sqrt_t = fast_sqrt(t);
        float log_ratio = fast_log(spot / strike_val);
        float xPowerTerm = 0.5f * vol * vol;
        float xD1 = (rt + xPowerTerm) * t + log_ratio;
        float xDen = vol * sqrt_t;
        xD1 = xD1 / xDen;
        float xD2 = xD1 - xDen;
        float NofXd1 = fast_norm_cdf(xD1);
        float NofXd2 = fast_norm_cdf(xD2);
        float FutureValueX = strike_val * fast_exp(-rt * t);
        p_call[i] = (spot * NofXd1) - (FutureValueX * NofXd2);
        p_put[i] = (FutureValueX * (1.0f - NofXd2)) - (spot * (1.0f - NofXd1));
    }
}
