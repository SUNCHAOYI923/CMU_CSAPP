...
void runScheduler(const std::string& name, const std::vector<std::string>& code) {
    ...
    std::cout << name << "\n";
    Parser parser;
    std::vector <Instruction> instructions = parser.parse (code);
    DAGBuilder builder;
    std::vector <DAGNode> nodes = builder.build (instructions);
    Scheduler scheduler;
    std::vector <TieBreakingPolicy> policies =
    {
        TieBreakingPolicy::SMALLER_INDEX,
        TieBreakingPolicy::LPT,
        TieBreakingPolicy::MOST_CHILD
    };
    std::vector<std::string> policyNames = {"SMALLER_INDEX","LPT","MOST_CHILD"};
    for (int i = 0;i < (int)policies.size ();++i)
    {
        std::cout << "\n--- Using policy: " << policyNames[i] << " ---\n";
        auto nodestmp = nodes;
        ScheduleResult result = scheduler.schedule (instructions, nodestmp, policies[i]);
        scheduler.printSchedule (result, instructions, nodestmp, std::cout);
        scheduler.printComparison (instructions, nodestmp, result, std::cout);
    }
