...
ScheduleResult Scheduler::schedule(const std::vector<Instruction>& instructions,
    ...
    int cycle = 0,N = (int) nodes.size ();
    std::vector <int> S;
    std::vector <std::pair <int,int>> inflight;
    for (auto &u : nodes) u.unscheduled_predecessors = (int)u.predecessors.size ();
    ...
    while ((int)S.size () < N)
    {
        ...
        if (!ready.empty ())
        {
            ...
            S.push_back (best);
            nodes[best].scheduled = true;nodes[best].schedule_cycle = cycle;
            inflight.push_back ({best,cycle + instructions[best].latency});
            for (auto &e : nodes[best].successors)
            {
                int v = e.target_node;
                if (e.type == DependencyType::WAR || e.type == DependencyType::WAW) --nodes[v].unscheduled_predecessors;
            }
        }
        ++cycle;
        ...
        {
            auto [op,t] = *it;
            if (t != cycle) {++it;continue;}
            it = inflight.erase (it);
            for (auto &e : nodes[op].successors)
            {
                int v = e.target_node;
                if (e.type == DependencyType::RAW) --nodes[v].unscheduled_predecessors;
            }
        }
    }
    result.order = S;result.total_cycles = cycle;
...
ScheduleResult Scheduler::scheduleFast(const std::vector<Instruction>& instructions,
    ...
    auto readyCmp = [&] (int a, int b) -> bool
    {
        ...
        switch (policy)
        {
            case TieBreakingPolicy::SMALLER_INDEX: return a > b;
            case TieBreakingPolicy::LPT: return instructions[a].latency != instructions[b].latency ? instructions[a].latency < instructions[b].latency : a > b;
            case TieBreakingPolicy::MOST_CHILD: return nodes[a].successors.size() != nodes[b].successors.size() ? nodes[a].successors.size() < nodes[b].successors.size() : a > b;
            default: return a > b;
        }
    };
    std::priority_queue <int, std::vector <int>, decltype (readyCmp) > ready (readyCmp);
    std::priority_queue <std::pair <int,int>> inflight; // early -> top
    for (int i = 0;i < N;++i)
        if (!nodes[i].unscheduled_predecessors) ready.push (i);
    while ((int)S.size () < N)
    {
        if (!ready.empty ())
        {
            int best = ready.top ();ready.pop ();
            S.push_back (best);
            nodes[best].scheduled = true;nodes[best].schedule_cycle = cycle;
            inflight.push ({-(cycle + instructions[best].latency),best});
            for (auto &e : nodes[best].successors)
            {
                int v = e.target_node;
                if (e.type == DependencyType::WAR || e.type == DependencyType::WAW)
                {
                    --nodes[v].unscheduled_predecessors;
                    if (!nodes[v].unscheduled_predecessors) ready.push (v);
                }
            }
        }
        ++cycle;
        while (!inflight.empty () && -inflight.top ().first == cycle)
        {
            auto [t,op] = inflight.top ();inflight.pop ();
            for (auto &e : nodes[op].successors)
            {
                int v = e.target_node;
                if (e.type == DependencyType::RAW) 
                {
                    --nodes[v].unscheduled_predecessors;
                    if (!nodes[v].unscheduled_predecessors) ready.push (v);
                }
...
int Scheduler::computeCriticalPath(int nodeIdx,
    ...
    if (visited[nodeIdx]) return nodes[nodeIdx].priority;
    visited[nodeIdx] = true;
    int tot_latency = instructions[nodeIdx].latency;
    for (auto &e : nodes[nodeIdx].successors)
    {
        int val = computeCriticalPath (e.target_node, instructions, nodes, visited);
        tot_latency = std::max (tot_latency,val + (e.type == DependencyType::RAW) * instructions[nodeIdx].latency);
    }
    // throw std::runtime_error("computeCriticalPath not implemented");
    return nodes[nodeIdx].priority = tot_latency;
}
        ...
        if (currentPriority > bestPriority) {
            ...
            switch (policy) {
                ...
                case TieBreakingPolicy::LPT:
                    best_L = instructions[bestNode].latency, curr_L = instructions[nodeIdx].latency;
                    if (curr_L > best_L) shouldReplace = true;
                    else if (curr_L == best_L) shouldReplace = (nodeIdx < bestNode);
                    break;
                case TieBreakingPolicy::MOST_CHILD:
                    best_L = nodes[bestNode].successors.size (), curr_L = nodes[nodeIdx].successors.size ();
                    if (curr_L > best_L) shouldReplace = true;
                    else if (curr_L == best_L) shouldReplace = (nodeIdx < bestNode);
                    break;
            }
