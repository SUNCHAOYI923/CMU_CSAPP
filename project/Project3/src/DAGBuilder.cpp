...
std::vector<DAGNode> DAGBuilder::build(const std::vector<Instruction>& instructions) {
    ...
    for (size_t i = 0; i < instructions.size(); i++)
    {
        ...
        if (inst.hasDestReg())
        {
            auto it1 = lastWriter.find (inst.dest_reg);
            if (it1 != lastWriter.end ())
            {
                int writerIdx = it1 -> second;
                addEdge (nodes, writerIdx, static_cast<int>(i), DependencyType::WAW, inst.dest_reg);
            }
            auto it2 = readers.find (inst.dest_reg);
            if (it2 != readers.end ())
            {
                for (auto readerIdx : it2 -> second) addEdge (nodes, readerIdx, static_cast<int>(i), DependencyType::WAR, inst.dest_reg);
                it2 -> second.clear ();
            }
        }
        for (const auto& srcReg : inst.src_regs) 
        {
            readers[srcReg].push_back (static_cast<int> (i));
        ...
        if (inst.hasDestReg()) lastWriter[inst.dest_reg] = static_cast<int>(i);
    }
...
void DAGBuilder::detectWAR(const std::vector<Instruction>& instructions,
    ...
    for (size_t i = 0; i < instructions.size(); i++) {
        ...
        if (inst.hasDestReg ())
        {
            auto it = readers.find (inst.dest_reg);
            if (it != readers.end ())
            {
                for (auto readerIdx : it -> second) addEdge(nodes, readerIdx, static_cast<int>(i), DependencyType::WAR, inst.dest_reg);
                it -> second.clear ();
            }
        }
        for (const auto& srcReg : inst.src_regs) readers[srcReg].push_back (static_cast<int>(i));
    }
